...
2. Add a user-facing endpoint to run and persist Perplexity research in one action:
   - Input (from user/UI): query, topic metadata, source tags, optional recency preference.
   - Backend action: call Perplexity, receive response content + citations, persist document, run extraction/indexing.
...
### Confidence threshold behavior for web extraction

1. Extraction computes/assigns an `extraction_confidence` score per thesis.
2. Default threshold: `0.70`.
3. If confidence >= 0.70: thesis is eligible for normal retrieval.
4. If confidence < 0.70: thesis is kept but marked low-confidence (hidden by default unless explicitly requested, or routed to review queue).
5. Confidence does not override source labels; all web-derived theses remain marked as time-sensitive opinion context.

---

## 13) Open Questions

1. TTL default window by source type (fund letters vs web research).
2. UX details for low-confidence thesis review (silent filter vs explicit review panel).
3. Whether to introduce scheduled Perplexity runs after manual-trigger flow stabilizes.
